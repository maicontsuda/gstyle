        ...
        <div className="card p-8 backdrop-blur-xl border-[var(--border)] shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border border-[var(--chrome-dark)]/30 bg-[var(--bg-card2)]/40 rounded-xl p-6 space-y-4">
                ...
                {equipamentosList.map(eq => (
                  <div key={eq}>
                    <label style={{
                      ...
                      background: formData.equipamentos.includes(eq) ? 'rgba(200,205,212,0.1)' : 'rgba(255,255,255,0.03)',
                      border: formData.equipamentos.includes(eq) ? '1px solid rgba(200,205,212,0.4)' : '1px solid rgba(255,255,255,0.06)',
                      transition: 'all 0.2s',
                    }}>
                      <input
                        type="checkbox"
                      ...
                      />
                      <span style={{ fontSize: '0.82rem', color: formData.equipamentos.includes(eq) ? 'var(--chrome-light)' : 'var(--text-muted)', userSelect: 'none', lineHeight: 1.3 }}>
                        {eq}
                      </span>
                    </label>

                    {/* Sub-opções de Aquecimento de Bancos */}
                    {eq === 'Aquecimento de Bancos' && formData.equipamentos.includes('Aquecimento de Bancos') && (
                      <div style={{ marginTop: '6px', marginLeft: '12px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                        ...
                        {aquecimentoOpcoes.map(opcao => {
                          const tag = `Aquecimento de Bancos - ${opcao}`;
                          const checked = formData.equipamentos.includes(tag);
                          return (
              ...
              {formData.equipamentos.filter(eq => !equipamentosList.includes(eq) && !aquecimentoOpcoes.map(o => `Aquecimento de Bancos - ${o}`).includes(eq)).length > 0 && (
                <div style={{ marginTop: '16px', paddingTop: '12px', borderTop: '1px solid rgba(138,144,153,0.2)' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--chrome-dark)', display: 'block', marginBottom: '8px' }}>Outros opcionais detectados:</span>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {formData.equipamentos.filter(eq => !equipamentosList.includes(eq) && !aquecimentoOpcoes.map(o => `Aquecimento de Bancos - ${o}`).includes(eq)).map(eq => (
