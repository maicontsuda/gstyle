...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (roleAuth.error) return NextResponse.json({ error: roleAuth.error }, { status: roleAuth.status });
    ...
    return NextResponse.json({ error: 'Erro ao adicionar anexos.' }, { status: 500 });
  }
}
